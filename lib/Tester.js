...
class Tester {
  ...
  */
   ...
   */
  isDisplayed() {
    return this.then(() => this._assertDisplayed(true));
  }
   ...
   */
  isNotDisplayed() {
    return this.then(() => this._assertDisplayed(false));
  }

  /**
   * @private
   */
  _assertDisplayed(expectedValue) {
    // If the element is not found at all, it is not visible.
    if (!expectedValue && this.currentElements.length === 0) {
      return true;
    }
    ...
    return this.currentElement
      .isDisplayed()
      .then(isDisplayed => {
        if (isDisplayed !== expectedValue) {
          throw new Error(`element "${this.currentElementsSelector}[${this.currentElementIndex}]" is${expectedValue ? ' not ' : ' '}displayed`);
        } else {
          return true;
        }
      });
  ...
  waitDisplayed(timeout) {
    return this.then(() => {
      return this.poll(() => {
        return this._assertDisplayed(true);
      }, value(timeout));
    });
  }

  /**
   * Wait until the current element is not visible. An error is thrown if the element is
   * visible after the implicit wait timeout. See `setImplicitWaitTimeout` method.
   */
  waitNotDisplayed(timeout) {
    return this.then(() => {
      return this.poll(() => {
        return this._assertDisplayed(false);
      }, value(timeout));
    });
  }

  /**
   * Assert that the current element is enabled.
   */
  isEnabled() {
    return this.then(() => this._assertEnabled(true));
  }

  /**
   * Assert that the current element is not enabled.
   */
  isDisabled() {
    return this.then(() => this._assertEnabled(false));
  }

  /**
   * @private
   */
  _assertEnabled(expectedValue) {
    return this.currentElement
      .isEnabled()
      .then(isEnbaled => {
        if (isEnbaled !== expectedValue) {
          throw new Error(`element "${this.currentElementsSelector}[${this.currentElementIndex}]" is${expectedValue ? ' not ' : ' '}enabled`);
        } else {
          return true;
        }
      });
  ...
  /**
   * Wait until the current element is disabled. An error is thrown if the element is not
   * disabled after the implicit wait timeout. See `setImplicitWaitTimeout` method.
   */
  waitDisabled(timeout) {
    return this.then(() => {
      return this.poll(() => {
        return this._assertEnabled(false);
      }, value(timeout));
    });
  }
   ...
   */
  isSelected() {
    return this.then(() => this._assertSelected(true));
  }

  /**
   * Assert that the current element is not selected.
   */
  isNotSelected() {
   ...
   * @private
   */
  _assertSelected(expectedValue) {
    return this.currentElement
      .isSelected()
      .then(isSelected => {
        if (isSelected !== expectedValue) {
          throw new Error(`element "${this.currentElementsSelector}[${this.currentElementIndex}]" is${expectedValue ? ' not ' : ' '}selected`);
        } else {
          return true;
        }
      });
  ...
  waitSelected(timeout) {
    return this.then(() => {
      return this.poll(() => {
        return this._assertSelected(true);
      }, value(timeout));
    });
  }

  /**
   * Wait until the current element is not selected. An error is thrown if the element is
   * selected after the implicit wait timeout. See `setImplicitWaitTimeout` method.
   */
  waitNotSelected(timeout) {
    return this.then(() => {
      return this.poll(() => {
        return this._assertSelected(false);
      }, value(timeout));
    });
  }
